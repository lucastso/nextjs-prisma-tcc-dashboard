...
const Sales = ({ sales }: SalesArrayProps) => {
  ...
  const soldThisMonth = sales
    .filter(
      (sale) =>
        sale.createdAt.slice(0, 7) === new Date().toISOString().slice(0, 7)
    )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid xs:grid-cols-2 lg:grid-cols-4 xs:gap-4 lg:gap-8">
        <div className="col-span-1 border rounded-md border-zinc-200 flex flex-col gap-4 p-4">
          <span>Hoje (dia {currentDay})</span>
          <span>
            R$ <strong className="text-2xl">{formatToPrice(soldToday)}</strong>
          </span>
        </div>
        <div className="col-span-1 border rounded-md border-zinc-200 flex flex-col gap-4 p-4">
        ...
        <div className="col-span-1 border rounded-md border-zinc-200 flex flex-col gap-4 p-4">
          <span>Total</span>
          <span>
            R$ <strong className="text-2xl">{formatToPrice(totalSold)}</strong>
          </span>
        </div>
      </div>
